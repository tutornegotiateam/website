...
class Descubre_Model extends CI_Model {
	
	private $descubres = 'descubre';
	function get_records() {		
		...
		//total records
		$sqlCount = 'SELECT COUNT(' . $indexColumn . ') AS row_count FROM ' . $this->descubres;
		...
		//final records
		$sql = 'SELECT SQL_CALC_FOUND_ROWS ' . str_replace(' , ', ' ', implode(', ', $columns)) . ' FROM ' . $this->descubres . $where . $order . $limit;
		...
		foreach ($result->result_array() as $cols) {
			...
			array_push($row, '<button class=\'edit btn btn-success btn-sm\'><i class=\'fa fa-pencil\'></i></button>&nbsp;&nbsp;<button class=\'delete btn btn-danger btn-sm\' id='. $cols[$indexColumn] .'><i class=\'fa fa-remove\'></i></button>');
            $output['data'][] = $row;
        }
	...
	function delete_record($id) {
		$sql = 'DELETE FROM ' . $this->descubres . ' WHERE id=' . $id;
		$this->db->query($sql);
	...
	function update_record($id, $titulo, $texto, $idioma, $finicio, $ffin,$activo,$usuario) {
		...
		$this->db->where('id', $id);
		$this->db->update($this->descubres, $data);
	...
	function add_record($name, $titulo, $texto, $idioma, $finicio, $ffin,$activo,$usuario) {
		$data = array(
					'titulo' => $titulo,
					'texto' => $price,
					'idioma' => $idioma,
		...
		$this->db->insert($this->descubres, $data);
