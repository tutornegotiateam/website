...
*/
	...
	public function _remap($method)
	{
		...
		if ($method==$secccion) {
			...
			$this->load->view('inicio/nav2',$data);
	           if($id_seccion=='contacto' && $id_contenido==''){ 
	                $data['title'] = 'Contacto';         	
			   	    $this->load->view('contacto/contacto_form',$data);
			   }elseif($id_seccion=='contacto' && $id_contenido=='cotizacion'){
			   	    $data['title'] = 'Cotización';  
			   	    $this->load->view('contacto/cotizacion',$data);
			   
			   }else{
			   	    $data['title'] = 'Sitio no encontrado';  
	...
	public function leer_cotizaciones()
	{
		$data = array();
		if ($this->session->userdata('lenguaje_website')=='' || $this->session->userdata('lenguaje_website')==null) {
			$idioma_seleccionado =$this->session->set_userdata('lenguaje_website', '1');
