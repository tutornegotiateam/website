...
<textarea class="form-control" name="sintesis" id="sintesis" placeholder="Entrada de articulo, nota o perpestiva"><?php echo !empty(@$member->sintesis)?@$member->sintesis:''; ?></textarea>
											<?php echo form_error('sintesis','<div class="invalid-feedback">','</div>'); ?>
...
<div class="col-sm-12">
<h3>Servicios relacionados a la Noticia:</h3>
<div class="col-sm-12" id="listados">
<?php if (!empty($arrSeccionesRel)) { foreach ($arrSeccionesRel as $r5) { 
if($ci->noticias_model->exiteRelacion( $r5['id'],@$member->id)>0){
?>
	<a href="javascript:void(0);" class="btn btn-secondary btn-xs" id="nota-<?php echo $r5['id']; ?>" onclick="marcarDesmarcar1(<?php echo $r5['id']; ?>,<?php echo @$member->id;?>)" ><?php echo $r5['titulo']; ?></a>
<?php }}} ?>
</div>
...
<tbody>
<?php
if (!empty($arrSeccionesRel)) { foreach ($arrSeccionesRel as $r5) {
	$seccion ='';
	if($r5['seccion']=='0'){
		$seccion = 'Principal';
	}else{
		$seccion = ucwords(str_replace('-',' ',$r5['seccion']));
	}
?>
	<tr>
		<td width="5%"><input type="checkbox" name="seleccionado[]" id="seleccionado-<?php echo $r5['id'];?>" <?php if($ci->noticias_model->exiteRelacion( $r5['id'],@$member->id)>0){ echo 'checked="checked"'; }  ?> onclick="marcarDesmarcar2(<?php echo $r5['id'];?>,<?php echo @$member->id;?>)" /></td>
		<td width="35%"><?php echo $seccion; ?></td>
		<td width="40%" id="txt-<?php echo $r5['id'];?>"><?php echo $r5['titulo']; ?></td>
		<td width="5%"><?php if( $r5['activo']==1){ echo "<div class='badge badge-success'>Activo</div>"; }else{  echo "<div class='badge badge-danger'>Inactivo</div>"; } ?></td>
		<td width="5%"><?php if($ci->noticias_model->exiteRelacion( $r5['id'],@$member->id)>0){ echo "<div class='badge badge-success'>SI</div>"; }else{  echo "<div class='badge badge-danger'>NO</div>"; } ?></td>
	</tr>
<?php }} ?>
</tbody>
...
<?php } ?>
									...
									<a href="<?php echo site_url('noticias_admin'); ?>" class="btn btn-secondary">
										Regresar
									</a>
...
<div id="servicios" class="modal fade" role="dialog" tabindex="-1">
...
$(".clickTd").click(function(e) {
        e.preventDefault();
        var data = $(this).parents("tr").find("td").eq(0).text();
        alert(data);    
});



function marcarDesmarcar1(a,b){

	
	
	bootbox.confirm({
    message: "Usted desea quitar la nota de la seccion seleccionada?",
    buttons: {
        confirm: {
            label: 'Si',
    ...
    callback: function (result) {
        if(result==true){
        	...
        	$.ajax({
            url: '<?php echo base_url();?>noticias_admin/eliminarRelacion',
            type: 'post',
            data: { idSeccion : a, idNota : b },
            success: function () {
                bootbox.alert("Se quito correctamente la nota");
                $("#seleccionado-"+a).prop( "checked", false);
	            $("#nota-"+a).remove();
            },
            error: function () {
			...
			$("#seleccionado-"+a).prop( "checked", true);
			//bootbox.alert("Ocurrio un error");
		}
    }
});
...
function marcarDesmarcar2(a,b){
	//a = seccion   b=noticia
	let valores = $('#txt-'+a).text();   
	if( $("#seleccionado-"+a).prop('checked') ) {
    // Seleccionado
     let  btn = '<a href="javascript:void(0);" class="btn btn-secondary btn-xs" id="nota-'+a+'" onclick="marcarDesmarcar1('+a+')" >'+valores+'</a>';
    
    
    bootbox.confirm({
    message: "Usted desea agregar la nota de la seccion seleccionada?",
    buttons: {
        confirm: {
            label: 'Si',
            className: 'btn-success'
    ...
    callback: function (result) {
        if(result==true){
        	...
        	$.ajax({
            url: '<?php echo base_url();?>noticias_admin/agregarRelacion',
            type: 'post',
            data: { idSeccion : a, idNota : b },
            success: function () {
                bootbox.alert("Se agrego correctamente la nota");
                $( "#listados" ).append( btn );
            },
            error: function () {
                bootbox.alert("Ocurrio un error");
...
$("#modal_servicios").click(function(ev) {
	ev.preventDefault();
 	var url = '<?php echo base_url(); ?>noticias_admin/notas_servicios';
 	//alert(url)
 	$('#servicios').modal('show').find('.modal-body').load(url);
