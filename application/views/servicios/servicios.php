<div style="background-color: #ececec;">
<section class="page-title-section bg-img cover-background"  data-overlay-dark="0" data-background="/assets/site/img/bg/bg_servicios.jpg">

    <div class="container">
        <div id="topic-description" class="topic-description">
						<h1 class="primary-headline">
						<?php echo $arrServiciosCategoria->ServicioTitulo;?>
						</h1>
						<h3><?php echo $arrServiciosCategoria->ServicioSubTitulo;?></h3>	
						<p class="body-copy"><?php echo $arrServiciosCategoria->ServicioContenido;?></p>
							
							         
					</div>
    </div>
    
</section>
<?php 
 $idc = str_replace('-','_',$idc);	   
 if($idc=='0'){ ?>
<div style="background-color: #e8eae9;">
<div class="row" style="background-color: #53565a">
<div class="container text-center">
	<ul id="servicios_css">
		<li><a href="/servicios/auditoria_ansurance/ifrs-en-mexico/" class="label">IFSR en Mexico</a></li>
		<li><a href="/servicios/auditoria_ansurance/servicios-de-asesoria-en-contabilidad-financiera/" class="label">Servicios de Asesoría en Contabilidad Financiera</a></li>
		<li><a href="/servicios/auditoria_ansurance/precios-de-transferencia-y-efectividad-del-modelo-operativo/" class="label">Precios de Transferencia y Efectividad del Modelo Operativo</a></li>
		<li><a href="/servicios/auditoria_ansurance/auditoria-de-estados-financieros/" class="label">Auditoría de Estados Financieros</a></li>
		<li><a href="/servicios/auditoria_ansurance/investigacion-de-fraudes-y-asistencia-en-litigios/" class="label">Investigación de Fraudes y Asistencia en Litigios</a></li>
	</ul>
</div>
</div>
</div>
<?php } ?>

<div class="container">
<div class="row" id="espacio_25"></div>
                <div class="row">
                    <div class="col-lg-9 col-md-12">
                     <div class="row">
                     
                     <?php 
                     $idc = str_replace('-','_',$idc);	   
                     if($idc=='0'){ ?>
                     
                     <div class="col-lg-12 col-md-12">
                     <div class="card_default " style="height: 250px">
                      <div class="card_default__content">
                            <div class="card_default__tags">
                                <span>
                                    Artículo
                                </span>
                            </div>
                            <h3 class="card_default__title">
                            <?php 
                            
                            if($idc==0){
								$url_ok =  base_url().strtolower($idioma_sele[0]->IdiomaSigla).'/servicios/'.$id.'/ver/1';
							}else{
								$url_ok =  base_url().strtolower($idioma_sele[0]->IdiomaSigla).'/servicios/'.$id.'/'.$idc.'/ver/1';
							}
                            
                            ?>
                                <a href="<?php echo $url_ok ?>">
                                    World Insurance Report 2019: Cómo superar los riesgos emergentes con tecnología
                                </a>
                            </h3>
                            <div class="card_default__text">
                                <p>
                                    A medida que los nuevos riesgos están afectando el panorama de los seguros, el World...
                                </p>
                            </div>
                            <div class="card_default__links">
                                <div class="card_default__more">
                                
                                    <a href="<?php echo $url_ok ?>" aria-label="">
                                        <svg xmlns="http://www.w3.org/2000/svg" width="36" height="36" viewBox="0 0 36 36">
                                            Leer más
                                            <g fill="none" fill-rule="evenodd" stroke="#0070AD" stroke-width="2">
                                                <path class="border_svg" d="M1 18c0 9.39 7.61 17 17 17s17-7.61 17-17S27.39 1 18 1 1 8.61 1 18z"></path>
                                                <path d="M15.018 11.997l3.958 3.958 2.042 2.042-6.021 6.02"></path>
                                            </g>
                                        </svg>
                                    </a>
                                </div>
                            </div>
                        </div>
                        <div class="card_default__media">                <!-- card_default__media -->
                            <a href="#">
                                World Insurance Report 2019: Cómo superar los riesgos emergentes con tecnología
                                <div class="card_default__media--inner" style="background-image: url(<?php echo base_url(); ?>uploads/publicaciones/01-01-Home-screen-sliders_1920x850_v1.jpg?w=540&;h=304&crop=1)">
				</div>                               
                            </a>
                        </div>
                     </div>
                     <div class="row" id="espacio_25"></div>
                    </div>
                    <?php }else{ 
                   // echo "entro";
                    ?>
					    <div class="col-lg-7 col-md-12">
                     <div class="card_default " style="height: 400px">
                     
                     <div class="card_default__content">
                            <div class="card_default__tags">
                                <span>
                                    Noticia
                                </span>
                            </div>
                            <h3 class="card_default__title">
                            <?php 
                            
                            if($idc==0){
								$url_ok =  base_url().strtolower($idioma_sele[0]->IdiomaSigla).'/servicios/'.$id.'/ver/1';
							}else{
								$url_ok =  base_url().strtolower($idioma_sele[0]->IdiomaSigla).'/servicios/'.$id.'/'.$idc.'/ver/1';
							}
                            
                            ?>
                                <a href="<?php echo $url_ok ?>">
                                    World Insurance Report 2019: Cómo superar los riesgos emergentes con tecnología
                                </a>
                            </h3>
                            <div class="card_default__text">
                                <p>
                                    A medida que los nuevos riesgos están afectando el panorama de los seguros, el World...
                                </p>
                            </div>
                            <div class="card_default__links">
                                <div class="card_default__more">
                                
                                    <a href="<?php echo $url_ok ?>" aria-label="">
                                        <svg xmlns="http://www.w3.org/2000/svg" width="36" height="36" viewBox="0 0 36 36">
                                            Leer más
                                            <g fill="none" fill-rule="evenodd" stroke="#0070AD" stroke-width="2">
                                                <path class="border_svg" d="M1 18c0 9.39 7.61 17 17 17s17-7.61 17-17S27.39 1 18 1 1 8.61 1 18z"></path>
                                                <path d="M15.018 11.997l3.958 3.958 2.042 2.042-6.021 6.02"></path>
                                            </g>
                                        </svg>
                                    </a>
                                </div>
                            </div>
                        </div>
                        <div class="card_default__media">                <!-- card_default__media -->
                            <a href="#">
                                World Insurance Report 2019 :Cómo superar los riesgos emergentes con tecnología
                               <!-- <div class="card_default__media--inner" style="background-image: url(<?php echo base_url(); ?>uploads/publicaciones/01-01-Home-screen-sliders_1920x850_v1.jpg?w=540&;h=304&crop=1)">
				</div>         -->                      
                            </a>
                        </div>
                     
                     
                     </div>
                     <div class="row" id="espacio_25"></div>
                    </div>
                 <!--   <div class="col-lg-3 col-md-12"><?php echo $id;?></div>-->
                    <div class="col-lg-5 col-md-12">
                    <div class="card_default " style="height: 400px">
                     
                     <div class="card_default__content">
                            <div class="card_default__tags">
                                <span>
                                    Artículo
                                </span>
                            </div>
                            <h3 class="card_default__title">
                            <?php 
                            
                            if($idc==0){
								$url_ok =  base_url().strtolower($idioma_sele[0]->IdiomaSigla).'/servicios/'.$id.'/ver/1';
							}else{
								$url_ok =  base_url().strtolower($idioma_sele[0]->IdiomaSigla).'/servicios/'.$id.'/'.$idc.'/ver/1';
							}
                            
                            ?>
                                <a href="<?php echo $url_ok ?>">
                                    World Insurance Report 2019: Cómo superar los riesgos emergentes con tecnología
                                </a>
                            </h3>
                            <div class="card_default__text">
                                <p>
                                    A medida que los nuevos riesgos están afectando el panorama de los seguros, el World...
                                </p>
                            </div>
                            <div class="card_default__links">
                                <div class="card_default__more">
                                
                                    <a href="<?php echo $url_ok ?>" aria-label="">
                                        <svg xmlns="http://www.w3.org/2000/svg" width="36" height="36" viewBox="0 0 36 36">
                                            Leer más
                                            <g fill="none" fill-rule="evenodd" stroke="#0070AD" stroke-width="2">
                                                <path class="border_svg" d="M1 18c0 9.39 7.61 17 17 17s17-7.61 17-17S27.39 1 18 1 1 8.61 1 18z"></path>
                                                <path d="M15.018 11.997l3.958 3.958 2.042 2.042-6.021 6.02"></path>
                                            </g>
                                        </svg>
                                    </a>
                                </div>
                            </div>
                        </div>
                        <div class="card_default__media">                <!-- card_default__media -->
                            <a href="#">
                                World Insurance Report 2019
                                <!--<div class="card_default__media--inner" style="background-image: url(<?php echo base_url(); ?>uploads/publicaciones/01-01-Home-screen-sliders_1920x850_v1.jpg?w=540&;h=304&crop=1)">
				</div>         -->                      
                            </a>
                        </div>
                     
                     
                     </div>
                     <div class="row" id="espacio_25"></div>
                    </div>	
					<?php } ?>
                    <?php /*
                     <div class="col-lg-8 col-md-12">
                     <div class="card_default " style="height: 250px">
                     a
                     </div>
                     <div class="row" id="espacio_25"></div>
                    </div>
                 <!--   <div class="col-lg-3 col-md-12"><?php echo $id;?></div>-->
                    <div class="col-lg-4 col-md-12">
                    <div class="card_default " style="height: 250px">
                     b
                     </div>
                     <div class="row" id="espacio_25"></div>
                    </div>
                    
         
                    <div class="col-lg-4 col-md-12">
                    <div class="card_default " style="height: 250px">
                     b
                     </div>
                     <div class="row" id="espacio_25"></div>
                    </div>
                    <div class="col-lg-4 col-md-12">
                    <div class="card_default " style="height: 250px">
                     b
                     </div>
                     <div class="row" id="espacio_25"></div>
                    </div>
                    <div class="col-lg-4 col-md-12">
                    <div class="card_default " style="height: 250px">
                     b
                     </div>
                     <div class="row" id="espacio_25"></div>
                    </div>*/?>
                      </div>
                     
                     
                    </div>
                    
                    <div class="col-lg-3 col-md-12">
                    	<div class="callback-box3">
                                <div class="section-heading half left white">
                                    <h5>Entremos en contácto</h5>
                                   <a href="javascript:void(0)" class="descarga_archivo"><span>Escribanos en nuestro formulario en línea</span></a>
                                </div>
                        </div>
                        <div class="margin-30px-top callback-box3">
                                <div class="section-heading half left white">
                                    <h5>Solicitud de cotización</h5>
                                    <a href="javascript:void(0)" class="descarga_archivo"><span>Enviar formulario electrónico</span></a>
                                </div>
                        </div>
                        <div class="margin-30px-top callback-box3">
                        	 <div class="row">
                        	<div class="col-12">
                        	<div class="widget">
                                <div class="widget-title">
                                    <h6><b>Noticias recientes</b></h6>
                                </div>
                                <div class="row">
                                	<div class="col-12">
                                <ul class="noticias_recientes">
                                <?php if(!empty($news_resent)){ foreach($news_resent as $r1){ ?>
                                    <li><a href="<?php echo base_url().strtolower(@$idioma_sele[0]->IdiomaSigla).'/'; ?>noticias/news/<?php echo $r1['id_txt']; ?>"><?php echo $r1['titulo']; ?></a></li>
                                <?php }} ?>
                                </ul>
                                	</div>                                	
                                </div>
                                <div class="widget-title">
                                    <h6><b><a href="<?php echo base_url().strtolower(@$idioma_sele[0]->IdiomaSigla).'/'; ?>noticias/">regresar a sala de prensa</a></b></h6>
                                </div>
                                
                            </div>  
                        	</div>
                        </div>
                        </div>
                        
                        
                        
                        
                        <div class="margin-30px-tb services-single-menu">
                        <ul class="no-margin list-style-none">
                        <li class="services-active"><a href="javascript:void(0)">Temas similares</a></li>
                        <li>
                        <a href="javascript:void(0)">En Breves IFRS</a>
                        <a href="javascript:void(0)">Professional Approach</a>
                        </li>                        
                        </ul>
                        </div>
                    </div>
                    </div>
                     </div>
                     </div>
                      <div id="linea-division">
<div class="container">

<div class="row">
    <div class="col-lg-6 col-md-12 sm-margin-20px-bottom">
        <div class="partner-box bg-white">
            <div class="row">
                <div class="col-md-4 col-sm-12">
                    <div class="partner-img">
                        <img alt="partner" src="/assets/site/img/personas/m-sin-foto.jpg" width="50%">
                    </div>
                </div>
                <div class="col-md-8 col-sm-12">
                    <div class="partner-text"><span>CO-Fundador</span>
                        <h4>Alfonso Ruelas</h4>
                        <p>aruelas@tutornegotia.com</p>
                        <ul class="social-icon-style1">
                            <li> <a href="javascript:void(0)"><i class="fab fa-facebook-f"></i></a></li>
                            <li> <a href="javascript:void(0)"><i class="fab fa-twitter"></i></a></li>
                            <li> <a href="javascript:void(0)"><i class="fab fa-google-plus-g"></i></a></li>
                            <li> <a href="javascript:void(0)"><i class="fab fa-linkedin-in"></i></a></li>
                        </ul>
                    </div>
                </div>
            </div>
        </div>
    </div>
    <div class="col-lg-6 col-md-12">
        <div class="partner-box bg-white">
            <div class="row">
                <div class="col-xs-12 col-md-4 col-sm-12">
                    <div class="partner-img">
                    <img alt="partner" src="/assets/site/img/personas/m-sin-foto.jpg" width="50%">
                    </div>
                </div>
                <div class="col-xs-12 col-md-8 col-sm-12">
                    <div class="partner-text"><span>Director auditorías</span>
                        <h4>Francisco Pestaño</h4>
                        <p>fpetano@tutornegotia.com</p>
                        <ul class="social-icon-style1">
                            <li> <a href="javascript:void(0)"><i class="fab fa-facebook-f"></i></a></li>
                            <li> <a href="javascript:void(0)"><i class="fab fa-twitter"></i></a></li>
                            <li> <a href="javascript:void(0)"><i class="fab fa-google-plus-g"></i></a></li>
                            <li> <a href="javascript:void(0)"><i class="fab fa-linkedin-in"></i></a></li>
                        </ul>
                    </div>
                </div>
            </div>
        </div>
    </div>
</div>




    </div>
</div>
</div>                    
<?php
